...
<h1 style="font-family: Monotype Corsiva; text-shadow: 0px 5px 5px #7496d6;">Ajouter Conjoint(e) Légitime</h1> <br>
...
<form method="post" action="{{route('conjoint.conjoint.store')}}" enctype="multipart/form-data">
    ...
    <div style="font-variant: small-caps; font-weight: bold;">
        ...
        <div class="form-group">
            ...
            <option value="{{ $agent->id }}" {{$conjoint->agent_id == $agent->id ? 'selected' : ''}} {{$idAgent ==  $agent->id ? 'selected' : ''}}> {{$agent->nomAgent}} {{$agent->prenomAgent}} </option>
            @endforeach
            </select>
            @error('agent_id')
